...
class AssistantBloc extends Bloc<AssistantEvent, AssistantState> {
  ...
  Future<void> _onOpened(AssistantOpened event, Emitter<AssistantState> emit) async {
    emit(state.copyWith(loading: true));
    final data = await repository.suggest();
    final welcome = AssistantMessage(isMe: false, text: data['welcome'] as String, at: DateTime.now());
    final actions = List<String>.from(data['actions'] as List);
    emit(state.copyWith(messages: [welcome], quickActions: actions, loading: false, connected: true));
  }

  Future<void> _onCleared(AssistantCleared event, Emitter<AssistantState> emit) async {
    emit(AssistantState.initial());
    add(AssistantOpened());
  }

  Future<void> _onSend(AssistantSendPressed event, Emitter<AssistantState> emit) async {
  ...
  Future<void> _onSuggest(AssistantSuggestionTapped event, Emitter<AssistantState> emit) async {
