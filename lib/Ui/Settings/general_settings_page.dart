...
import 'widgets/tiles/tile_switch.dart';
...
class GeneralSettingsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (ctx) => SettingsBloc(ctx.read<ISettingsRepository>())..add(SettingsOpened()),
      child: const _Body(),
    );
  }
}
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Align(alignment: Alignment.centerRight, child: Text('الإعدادات العامة', textAlign: TextAlign.right)),
        backgroundColor: Colors.white,
        elevation: 0,
      ...
      body: SafeArea(
        child: BlocConsumer<SettingsBloc, SettingsState>(
          listener: (context, state) {
            if (state.saveOk) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ التغييرات'), backgroundColor: Color(0xFF16A34A)));
            }
          },
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    TileChoiceChips(
                      ...
                      onExpand: () async {
                        final v = await showLanguagePickerSheet(context, current: m.language);
                                                 if (v != null) {
                           bloc.add(OptionChanged('language', v));
                           bloc.add(SaveRequested());
                         }
                    ...
                    TileChoiceChips(
                      ...
                      value: m.theme,
                                             options: choiceOptions(const [
                         {'label': 'فاتح', 'icon': Icons.light_mode_rounded},
                         {'label': 'داكن', 'icon': Icons.dark_mode_rounded},
                       ]),
                                             onChanged: (v) {
                      ...
                      onExpand: () async {
                        final v = await showThemePickerSheet(context, current: m.theme);
                                                 if (v != null) {
                           bloc.add(OptionChanged('theme', v));
                           bloc.add(SaveRequested());
                         }
                  ...
                  _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    const SizedBox(height: 12),
                                         TileSelect(title: 'التقويم', value: m.region.calendar, options: const ['هجري/ميلادي', 'ميلادي فقط'], onChanged: (v) {
                       bloc.add(OptionChanged('region.calendar', v));
                       bloc.add(SaveRequested());
                     }),
                    const SizedBox(height: 12),
                                         TileSelect(title: 'تنسيق الوقت', value: m.region.timeFormat, options: const ['24 ساعة', '12 ساعة'], onChanged: (v) {
                       bloc.add(OptionChanged('region.timeFormat', v));
                       bloc.add(SaveRequested());
                     }),
                    const SizedBox(height: 12),
                                         TileSelect(title: 'أول يوم في الأسبوع', value: m.region.weekStart, options: const ['السبت', 'الأحد', 'الاثنين'], onChanged: (v) {
                  ...
                  _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    const SizedBox(height: 12),
                    InfoRow(label: 'رقم البِناء', value: m.about.build.toString()),
                    const SizedBox(height: 12),
                    TileNav(title: 'اتفاقية الاستخدام', onTap: () => Navigator.of(context).pushNamed('/about/tos')),
                    const SizedBox(height: 12),
                    TileNav(title: 'سياسة الخصوصية', onTap: () => Navigator.of(context).pushNamed('/about/privacy')),
                  ])),
...
class _SectionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(color: Colors.white, border: Border.all(color: const Color(0xFFE6EAF2)), borderRadius: BorderRadius.circular(16)),
      padding: const EdgeInsets.all(16),
      child: child,
    );
  }
