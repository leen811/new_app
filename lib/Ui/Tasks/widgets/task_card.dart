...
class TaskCard extends StatelessWidget {
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
            ...
            Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              if (item.isRunning)
                ...
                _SquareButton(icon: Icons.play_arrow, onTap: onTimerStart),
            ]),
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            if (item.tags.isNotEmpty) Wrap(spacing: 6, runSpacing: -6, children: item.tags.map((t) => _chip(t)).toList()),
            const SizedBox(height: 10),
            Row(children: [
              const Icon(Icons.event, size: 14, color: Color(0xFF98A2B3)),
              const SizedBox(width: 4),
              Text(item.intlTimeLabel.isEmpty ? 'بدون تاريخ تسليم' : item.intlTimeLabel, style: const TextStyle(fontSize: 12, color: Color(0xFF667085))),
              const Spacer(),
              _CircleIconButton(icon: Icons.more_horiz, onTap: () => _openDetailsDialog(context, item)),
            ])
          ]),
        )
      ]),
  ...
  Widget _priorityBadge(String p) {
    ...
    return Container(
      ...
      child: Text(p, style: const TextStyle(fontSize: 11, color: Colors.white, fontWeight: FontWeight.w600)),
    );
  }
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Icon(checked ? Icons.check : Icons.crop_square, size: 16, color: checked ? Colors.white : const Color(0xFF334155)),
      ),
    );
  }
}
...
class _TimeChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text, style: const TextStyle(fontSize: 11, color: Color(0xFF23408A), fontWeight: FontWeight.w600)),
    );
  }
}
              ...
              child: Padding(
                ...
                child: StatefulBuilder(builder: (context, setState) {
                  final rangeLabel = range == null
                      ? (item.intlTimeLabel.isEmpty ? 'اختر المدة' : item.intlTimeLabel)
                  ...
                  return Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    Row(children: [
                      const Expanded(child: Text('تفاصيل المهمة', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Colors.black))),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (v) => setState(() => priority = v ?? item.priority),
                    ),
                        ...
                        child: AppButton(
                          ...
                          onPressed: () {
                            ...
                            final upd = TaskItem(
                              ...
                              intlTimeLabel: label,
                              category: selectedWorks.isNotEmpty ? selectedWorks.first : item.category,
                              done: item.done,
                              timerSeconds: item.timerSeconds,
                              isRunning: item.isRunning,
                            );
                            pageContext.read<DailyTasksBloc>().add(DailyTaskUpdated(upd));
                            Navigator.pop(ctx);
                          },
                        ),
                      ),
